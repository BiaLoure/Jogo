		...
		for node in nodes:
			if node is SaveDataComponent:
				...
				if save_data_resource != null:
					...
					push_error("SaveDataComponent %s não retornou recurso válido" % node.name)

func save_game() -> void:
	if !DirAccess.dir_exists_absolute(save_game_data_path):
		var err := DirAccess.make_dir_recursive_absolute(save_game_data_path)
		if err != OK:
			push_error("Erro ao criar pasta: %s (código %d)" % [save_game_data_path, err])
