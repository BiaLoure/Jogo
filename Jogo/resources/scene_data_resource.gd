...
func _load_data(root: Node) -> void:
	...
	if parent_node_path != null:
		parent_node = root.get_node_or_null(parent_node_path)
		
	if node_path != null:
		var scene_file_resources: Resource = load(scene_file_path)
		scene_node = scene_file_resources.instantiate() as Node2D
